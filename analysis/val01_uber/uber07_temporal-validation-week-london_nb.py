...
g = sns.histplot(
    hourly_count[(hourly_count["date_hour"] >= datetime(2019, 10, 22)) & (hourly_count["date_hour"] < datetime(2019, 10, 23))],
    x="date_hour",
    hue="label",
...
for idx, day in enumerate(range(START_DAY, END_DAY + 1)):
    ...
    tspeeds_highway_hourly_sum_pivot = tspeeds_highway_hourly_sum[
        (tspeeds_highway_hourly_sum["date_hour"] >= datetime(2019, 10, day)) & (tspeeds_highway_hourly_sum["date_hour"] < datetime(2019, 10, day + 1))
...
for idx, day in enumerate(range(START_DAY, END_DAY + 1)):
    ...
    uspeeds_highway_hourly_sum_pivot = uspeeds_highway_hourly_sum[
        (uspeeds_highway_hourly_sum["date_hour"] >= datetime(2019, 10, day)) & (uspeeds_highway_hourly_sum["date_hour"] < datetime(2019, 10, day + 1))
    ].pivot("highway", "date_hour", "count")
...
for idx, day in enumerate(range(START_DAY, END_DAY + 1)):
    ...
    matched_highway_hourly_sum_pivot = matched_highway_hourly_sum[
        (matched_highway_hourly_sum["date_hour"] >= datetime(2019, 10, day)) & (matched_highway_hourly_sum["date_hour"] < datetime(2019, 10, day + 1))
    ].pivot("highway", "date_hour", "count")
