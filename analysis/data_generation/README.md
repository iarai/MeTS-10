...
* `01_download_all.sh` downloads to `here_downloads`
* `02_extract_all.sh` extracts from `here_downloads` to `here_extracted` and links `<year>/movie` to a subfolder of `here_extracted`
* `03_create_release.sh` create RELEASE subfolder
* `04_link_movie15min.sh` optionally link to existing `movie_15min` and subfolder if `dp
* `05_run_all.sh` works under `<year>/movie` as data dir for the pipeline
