...
def row_col_for_lon_lat(lon, lat, lon_min, lat_min, rotate):
    ...
    if rotate:
        row = NUM_ROWS - 1 - degree_to_grid(lon, lon_min)
        column = NUM_COLUMNS - 1 - degree_to_grid(lat, lat_min)
    else:
        row = NUM_ROWS - 1 - degree_to_grid(lat, lat_min)
        column = degree_to_grid(lon, lon_min)
    return row, column
...
def create_polygon_from_row_col(row, col, lon_min, lat_min, rotate) -> Polygon:
    ...
    if rotate:
        cell_lat_min = lat_min + (((NUM_COLUMNS - 1 - col) * 100) / 100000)
