...
Arguments:
  ...
  -a, --aggreagation MOVIE_SUBFOLDER
                        Movie subfolder for the aggregation to be used. Defaults to `movie_15min`
                        Aggregation to be created. Must be a multiple of 5. Will create subfolder movie_<aggregation>min
...
def generate_aggregate(data: np.ndarray, aggregation=15):
    ...
    NUM_CONSECUTIVE_5min = int(aggregation // 5)
    data_resized = data.reshape(NUM_SLOTS_AGGREGATED, NUM_CONSECUTIVE_5min, NUM_ROWS, NUM_COLUMNS, 8)
...
def generate_aggregates(data_folder: Path, aggregation=15, city=None, resume=False):
    if city:
    ...
    for input_fn in input_files:
        output_name = input_fn.name.replace("8ch", f"8ch_{aggregation}min").replace("9ch", f"8ch_{aggregation}min")

        output_fn = data_folder / f"movie_{aggregation}min" / input_fn.relative_to(data_folder / "movie").parent / output_name
        ...
        data_aggregated = generate_aggregate(data, aggregation=aggregation)
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "--aggregation",
        type=int,
        help="Aggregation to be used. Must be a multiple of 5. Will create subfolder movie_<aggregation>min",
        required=False,
        default=15,
    )
...
def main(argv):
    ...
    try:
        ...
        assert aggregation % 5 == 0, f"Aggregation must be multiple of 5, found {aggregation}."
    except Exception as e:
        logging.exception(f"Could not parse args.", exc_info=e)
        parser.print_help()
        exit(1)
    generate_aggregates(data_folder, city=city, resume=resume, aggregation=aggregation)
