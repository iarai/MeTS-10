...
def process_intersecting_cells(data_folder: Path, city: str, overwrite=False):
    ...
    # check key constraint before writing
    df_by_key = cells_df.groupby(["gkey"]).agg(count=("geometry", "count"))
    assert df_by_key["count"].max() == 1, f'gkey not unique: {df_by_key[df_by_key["count"] > 1]}'
