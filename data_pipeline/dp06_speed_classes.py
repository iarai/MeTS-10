...
Arguments:
  ...
  -ntf --no_trust_filtering
                        Disable filtering by trust (based on congestion factor = median speed / free flow). By default, trust filtering is enabled.
  -dffn, --disable_free_flow_normalization
                        Disable normalizing free flow speeds using the speed limit. Applies only if trust filtering is enabled. By default, free flow normalization is enabled.
  -a, --aggreagation MINUTES
                        Aggregation to be used. Must be a multiple of 5. Will use subfolder movie_<aggregation>min. Defaults to 15.
...
SegmentSpeedStats = namedtuple(
    "SegmentSpeedStats", ["volumes", "speeds", "median_speeds", "median_speeds_kph", "mean_speeds", "mean_speeds_kph", "std_speeds", "std_speeds_kph"]
)
...
def extract_segment_speeds(data_aggregated_day, intersecting_cells):
    ...
    with warnings.catch_warnings():
        ...
        std_speeds = np.nanstd(speeds, axis=0)
    median_speeds_kph = median_speeds / 255 * MOVIE_MAXSPEED_KPH
    mean_speeds_kph = mean_speeds / 255 * MOVIE_MAXSPEED_KPH
    speeds_kph = speeds / 255 * MOVIE_MAXSPEED_KPH
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        std_speeds_kph = np.nanstd(speeds_kph, axis=0)
    return SegmentSpeedStats(vols, speeds, median_speeds, median_speeds_kph, mean_speeds, mean_speeds_kph, std_speeds, std_speeds_kph)
...
def extract_sc_features(edges_df, date, data_aggregated_day, normalize_free_flow=True, no_trust_filtering=False, slot_offset=0):  # noqa: C901
    road_graph_size = len(edges_df)
    result_data = []
    slots = data_aggregated_day.shape[0]
    for i, u, v, intersecting_cells, free_flow_kph, speed_limit_kph, gkey, osmid in zip(
        range(road_graph_size),
        edges_df["u"],
        edges_df["v"],
        edges_df["intersecting_cells"],
        edges_df["free_flow_kph"],
        edges_df["speed_kph"],
        edges_df["gkey"],
        edges_df["osmid"],
    ):
        ...
        for t in range(slots):
            ...
            d = {
                ...
                "mean_speed_kph": speed_stats.mean_speeds_kph[t],
                "std_speed_kph": speed_stats.std_speeds_kph[t],
...
def process_speed_classes(
    data_folder: Path, city: str, resume=False, normalize_free_flow=True, no_trust_filtering=False, aggregation: int = 15, gpkg_date=None
):
    ...
    input_files = sorted((data_folder / f"movie{aggregation_suffix}" / city).rglob(f"*_8ch{aggregation_suffix}.h5"))
    ...
    for input_fn in input_files:
        ...
        data = extract_sc_features(edges_df, date, data_aggregated_day, normalize_free_flow=normalize_free_flow, no_trust_filtering=no_trust_filtering)
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "-ntf",
        "--no_trust_filtering",
        help="Disable normalizing free flow speeds using the speed limit. Applies only if trust filtering is enabled.",
        required=False,
        action="store_true",
        default=False,
    )
    ...
    parser.add_argument(
        "-dffn",
        "--disable_free_flow_normalization",
        help="Disable normalizing free flow speeds using the speed limit. Applies only if trust filtering is enabled.",
        required=False,
        action="store_true",
    )
    parser.add_argument(
        ...
        "--aggregation",
        type=int,
        help="Aggregation to be used. Must be a multiple of 5. Will use subfolder movie_<aggregation>min.",
        required=False,
        default=15,
    )
    parser.add_argument(
...
def main(argv):
    ...
    try:
        ...
        no_trust_filtering = params["no_trust_filtering"]
        gpkg_date = params["gpkg_date"]
    except Exception as e:
        logging.exception(f"Could not parse args.", exc_info=e)
        parser.print_help()
        exit(1)
    process_speed_classes(
        data_folder,
        city=city,
        resume=resume,
        no_trust_filtering=no_trust_filtering,
        normalize_free_flow=normalize_free_flow,
